...
export default function LoginScreen() {
  ...
  const checkStatus = async (pubKey: string) => {
    ...
    try {
      // Uložíme adresu trvalo do zariadenia
      await AsyncStorage.setItem('wallet_addr', pubKey);
      ...
      if (info) router.replace({ pathname: '/dashboard', params: { wallet: pubKey } })
      else router.replace({ pathname: '/merchant-setup', params: { wallet: pubKey } })
    } catch (e) {
      router.replace({ pathname: '/dashboard', params: { wallet: pubKey } })
    }
  }
  ...
  const handleConnect = async () => {
    ...
    } catch (e) { setStatus(''); setConnecting(false) }
  }

  return (
    <View style={styles.container}>
      <StatusBar style="light" /><LinearGradient colors={['#111827', '#030712']} style={StyleSheet.absoluteFill} />
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.content}>
          <Text style={styles.title}>SubscribeSol</Text>
          <Text style={styles.subtitle}>Merchant Portal</Text>
          <View style={styles.descriptionContainer}><Text style={styles.description}>Connect your wallet to manage your dashboard.</Text></View>
